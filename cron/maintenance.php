...
$stats=['reminders'=>0,'retried_jobs'=>0,'temp_files_removed'=>0,'backup'=>'skipped'];

try{$existingBackups=listBackups();if(!$existingBackups||date('Y-m-d',$existingBackups[0]['created_at'])!==date('Y-m-d')){$backup=createDatabaseBackup($pdo);$stats['backup']=basename($backup['database']);}}catch(Throwable $error){$stats['backup']='error: '.$error->getMessage();}

$stmt=$pdo->query("SELECT DISTINCT a.id,a.title,a.due_date,lcs.student_id FROM assignments a JOIN learning_classes lc ON lc.course_id=a.course_id AND lc.status='active' JOIN learning_class_students lcs ON lcs.learning_class_id=lc.id WHERE a.due_date BETWEEN NOW() AND DATE_ADD(NOW(),INTERVAL 24 HOUR) AND NOT EXISTS (SELECT 1 FROM submissions s WHERE s.assignment_id=a.id AND s.student_id=lcs.student_id) AND NOT EXISTS (SELECT 1 FROM notifications n WHERE n.user_id=lcs.student_id AND n.type='assignment_due_reminder' AND JSON_UNQUOTE(JSON_EXTRACT(n.data_json,'$.assignment_id'))=CAST(a.id AS CHAR))");
