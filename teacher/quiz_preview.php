...
<section class="box"><h2><i class='bx bx-show'></i> Nội dung đề</h2><?php foreach($questions as $index=>$question):?><?php $correctOptions=quizAnswerOptions($question['correct_option']);?><article class="preview-question"><strong>Câu <?php echo $index+1;?>. <?php echo nl2br(htmlspecialchars($question['question_text']));?></strong><?php if(count($correctOptions)>1):?><div style="margin-top:7px;color:var(--primary);font-size:13px;font-weight:700">Nhiều đáp án đúng: <?php echo htmlspecialchars(implode(', ', $correctOptions));?></div><?php endif;?><div class="preview-options"><?php foreach(['A','B','C','D'] as $letter):?><div class="preview-option <?php echo in_array($letter,$correctOptions,true)?'correct':'';?>"><strong><?php echo $letter;?>.</strong> <?php echo htmlspecialchars($question['option_'.strtolower($letter)]);?></div><?php endforeach;?></div></article><?php endforeach;?><?php if(!$questions):?><p style="color:var(--text-muted)">Đề chưa có câu hỏi.</p><?php endif;?></section>
