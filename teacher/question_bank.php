...
$redirect = static function (): never { header('Location: question_bank.php'); exit; };
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create_topic') {
            ...
            if ($duplicate->fetchColumn()) throw new RuntimeException('Nội dung sau khi sửa bị trùng với một câu hỏi khác.');
            ...
            if($distributionTotal>0){foreach($difficultyCounts as $level=>$levelCount){if($levelCount===0)continue;$levelStmt=$pdo->prepare('SELECT * FROM question_bank WHERE '.implode(' AND ',$baseConditions).' AND difficulty=? ORDER BY RAND() LIMIT '.$levelCount);$levelStmt->execute([...$baseParams,$level]);$levelRows=$levelStmt->fetchAll();if(count($levelRows)<$levelCount)throw new RuntimeException('Không đủ câu mức '.questionDifficultyLabel($level).'. Hiện có '.count($levelRows).' câu.');$selected=array_merge($selected,$levelRows);}$count=$distributionTotal;shuffle($selected);}else{$conditions=$baseConditions;$params=$baseParams;if(in_array($difficulty,['easy','medium','hard'],true)){$conditions[]='difficulty=?';$params[]=$difficulty;}$bank=$pdo->prepare('SELECT * FROM question_bank WHERE '.implode(' AND ',$conditions).' ORDER BY RAND() LIMIT '.$count);$bank->execute($params);$selected=$bank->fetchAll();if(count($selected)<$count)throw new RuntimeException('Ngân hàng không đủ câu hỏi phù hợp. Hiện có '.count($selected).' câu.');}
            ...
            $insert=$pdo->prepare('INSERT INTO quiz_questions (section_id,source_question_id,question_text,option_a,option_b,option_c,option_d,correct_option,sort_order) VALUES (?,?,?,?,?,?,?,?,?)');$usage=$pdo->prepare('UPDATE question_bank SET usage_count=usage_count+1 WHERE id=?');foreach($selected as $i=>$q){$insert->execute([$sectionId,$q['id'],$q['question_text'],$q['option_a'],$q['option_b'],$q['option_c'],$q['option_d'],$q['correct_option'],$i+1]);$usage->execute([$q['id']]);}$pdo->commit();writeAuditLog($pdo,'quiz.generated_from_bank','quiz',$quizId,['question_count'=>$count,'shuffle_questions'=>true,'shuffle_options'=>true,'is_published'=>true]);$_SESSION['success']='Đã tạo, trộn câu hỏi, trộn đáp án và mở đề cho học viên làm.';
        }
    } catch(Throwable $error){if($pdo->inTransaction())$pdo->rollBack();$_SESSION['error']=$error->getMessage();}
    $redirect();
}

$topicsSql=$isAdmin&&$ownerId===0?'SELECT * FROM question_topics ORDER BY name':'SELECT * FROM question_topics WHERE teacher_id=? ORDER BY name';$topicsStmt=$pdo->prepare($topicsSql);$topicsStmt->execute($isAdmin&&$ownerId===0?[]:[$ownerId?:$actorId]);$topics=$topicsStmt->fetchAll();
...
$stmt=$pdo->prepare('SELECT qb.*,qt.name topic_name,u.name teacher_name FROM question_bank qb LEFT JOIN question_topics qt ON qt.id=qb.topic_id LEFT JOIN users u ON u.id=qb.teacher_id WHERE '.$whereSql.' ORDER BY qb.updated_at DESC LIMIT '.$perPage.' OFFSET '.$offset);$stmt->execute($params);$questions=$stmt->fetchAll();
$questionVersions=[];
...
<section class="bank-panel" style="margin-bottom:20px"><h2><i class='bx bx-purchase-tag'></i> Quản lý chủ đề</h2><div style="display:grid;grid-template-columns:minmax(280px,420px) 1fr;gap:18px;align-items:start"><form method="post" class="bank-form"><?php echo csrfField();?><input type="hidden" name="action" value="create_topic"><?php if($isAdmin):?><select name="teacher_id" required><option value="">Chọn giảng viên phụ trách</option><?php foreach($teachers as $teacher):?><option value="<?php echo $teacher['id'];?>"><?php echo htmlspecialchars($teacher['name']);?></option><?php endforeach;?></select><?php endif;?><input name="topic_name" placeholder="Nhập tên chủ đề mới" maxlength="191" required><button class="btn btn-primary"><i class='bx bx-plus'></i> Tạo chủ đề</button></form><div class="bank-actions"><?php foreach($topics as $topic):?><span style="display:inline-flex;align-items:center;gap:7px;padding:8px 10px;border:1px solid var(--border-color);border-radius:999px;background:var(--input-bg)"><?php echo htmlspecialchars($topic['name']);?><form method="post" onsubmit="return confirm('Xóa chủ đề này?')"><?php echo csrfField();?><input type="hidden" name="action" value="delete_topic"><input type="hidden" name="topic_id" value="<?php echo $topic['id'];?>"><button type="submit" title="Xóa chủ đề" style="border:0;background:none;color:var(--danger);cursor:pointer;padding:0"><i class='bx bx-x'></i></button></form></span><?php endforeach;?><?php if(!$topics):?><span style="color:var(--text-muted)">Chưa có chủ đề. Hãy tạo chủ đề đầu tiên.</span><?php endif;?></div></div></section>
