        ...
        try {
            ...
            if ($action === 'move_quiz') {
                ...
                if ($isAjaxRequest) {
                    $imageUrls = [];
                    foreach ($imageFields as $imageField) {
                        $path = (string) ($images[$imageField] ?? '');
                        $imageUrls[$imageField] = $path === ''
                            ? null
                            : '../uploads/' . implode('/', array_map('rawurlencode', explode('/', $path)));
                    }
                    header('Content-Type: application/json; charset=utf-8');
                    echo json_encode([
                        'success' => true,
    ...
    .question-editor{padding:16px;border:1px solid var(--border-color);border-radius:12px;margin-top:12px}.question-editor.is-saving{opacity:.78}.answer-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
    .question-save-status{min-height:20px;display:inline-flex;align-items:center;font-size:13px;color:var(--text-muted)}.question-save-status.saving{color:#fbbf24}.question-save-status.saved{color:var(--success)}.question-save-status.error{color:var(--danger)}
    ...
    (() => {
        ...
        const updateImageTool = (form, field, imageUrl) => {
            const fileInput = form.querySelector(`input[type="file"][name="${field}"]`);
            const tools = fileInput?.closest('.image-tools');
            if (!fileInput || !tools) return;

            tools.querySelector('.question-image')?.remove();
            const oldRemoveInput = tools.querySelector(`input[name="remove_${field}"]`);
            oldRemoveInput?.closest('label')?.remove();

            if (imageUrl) {
                const image = document.createElement('img');
                image.className = 'question-image';
                image.src = imageUrl;
                image.alt = field === 'question_image' ? 'Ảnh câu hỏi' : 'Ảnh đáp án';

                const removeLabel = document.createElement('label');
                const removeInput = document.createElement('input');
                removeInput.type = 'checkbox';
                removeInput.name = `remove_${field}`;
                removeLabel.append(removeInput, document.createTextNode(' Xóa ảnh hiện tại'));

                const uploadLabel = [...tools.children].find(element => element.tagName === 'LABEL') || fileInput;
                tools.insertBefore(image, uploadLabel);
                tools.insertBefore(removeLabel, uploadLabel);
            }
        ...
        document.querySelectorAll('form.question-editor').forEach(form => {
            const saveButton = form.querySelector('.inline-actions .btn-primary');
            const deleteButton = form.querySelector('button[name="action"][value="delete_question"]');
            if (!saveButton) return;

            const status = document.createElement('span');
            status.className = 'question-save-status';
            status.setAttribute('role', 'status');
            status.setAttribute('aria-live', 'polite');
            saveButton.closest('.inline-actions')?.appendChild(status);

            let clearStatusTimer = null;
            const setStatus = (message, state = '') => {
                window.clearTimeout(clearStatusTimer);
                status.textContent = message;
                status.className = `question-save-status ${state}`.trim();
            };

            form.addEventListener('submit', async event => {
                if (event.submitter?.value === 'delete_question') return;
                event.preventDefault();

                const originalButtonHtml = saveButton.innerHTML;
                saveButton.disabled = true;
                if (deleteButton) deleteButton.disabled = true;
                saveButton.innerHTML = "<i class='bx bx-loader-alt bx-spin'></i> Đang lưu";
                form.classList.add('is-saving');
                form.setAttribute('aria-busy', 'true');
                setStatus('Đang lưu thay đổi...', 'saving');
                    ...
                    try {
                        ...
                        throw new Error(response.ok
                            ...
                            : (isHtmlResponse
                                ...
                                : (responseText.trim() || 'Không thể lưu câu hỏi.')));
                    }
                    if (!response.ok || !result.success) {
                        throw new Error(result.message || 'Không thể lưu câu hỏi.');
                    }

                    imageFields.forEach(field => updateImageTool(form, field, result.images?.[field] || null));
                    setStatus(result.message || 'Đã lưu câu hỏi.', 'saved');
                    clearStatusTimer = window.setTimeout(() => setStatus(''), 2500);
                } catch (error) {
                    setStatus(error.message || 'Không thể lưu câu hỏi.', 'error');
                } finally {
